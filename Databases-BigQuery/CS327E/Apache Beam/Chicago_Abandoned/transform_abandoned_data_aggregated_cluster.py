...
options = {
    ...
    'job_name': 'transform-abandoned-data',
    'project': PROJECT_ID,
...
with beam.Pipeline('DataflowRunner', options=opts) as p:
    ...
    query_results = p | 'Read from BigQuery' >> beam.io.Read(beam.io.BigQuerySource(query='SELECT * FROM chicago_abandoned_2001.abandoned_data_aggregated LIMIT 100'))
    ...
    #grouped_pcoll = request_pcoll | 'Group by Request_Number' >> beam.GroupByKey()

    # write PCollection to log file
    #grouped_pcoll | 'Write log 3' >> WriteToText(DIR_PATH + 'grouped_pcoll.txt')
